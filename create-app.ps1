<#
.SYNOPSIS
    Creates the app.py file with the FastAPI wrapper for PubMed MCP Server
.DESCRIPTION
    This script creates the FastAPI application that exposes the MCP server as REST endpoints
#>
...
Write-Host "🚀 Creating app.py - FastAPI REST API wrapper..." -ForegroundColor Green

# App.py content with FastAPI implementation
$appContent = @'
"""
...
$fullPath = Join-Path $WorkspacePath "app.py"
Set-Content -Path $fullPath -Value $appContent -Encoding UTF8

Write-Host "✅ Created: app.py - FastAPI REST API wrapper (Complete implementation)" -ForegroundColor Green
Write-Host "📄 File size: $([math]::Round((Get-Item $fullPath).Length / 1KB, 1)) KB" -ForegroundColor Cyan
Write-Host "🔧 Features included:" -ForegroundColor Yellow
Write-Host "   • Full OpenAPI 3.0 specifications" -ForegroundColor White
Write-Host "   • Interactive API documentation" -ForegroundColor White
Write-Host "   • Request/Response validation" -ForegroundColor White
...
Write-Host "   • Azure Foundry ready" -ForegroundColor White
...
Write-Host "   • POST /search     - Search PubMed" -ForegroundColor White
Write-Host "   • POST /publication - Get publication details" -ForegroundColor White
Write-Host "   • POST /similar    - Find similar articles" -ForegroundColor White
Write-Host "   • GET  /tools      - List available tools" -ForegroundColor White
Write-Host "   • GET  /docs       - Interactive documentation" -ForegroundColor White
Write-Host "   • GET  /openapi.json - OpenAPI 3.0 specification" -ForegroundColor White

Write-Host "`n🎉 app.py created successfully!" -ForegroundColor Green
Write-Host "📋 Next: Create Azure deployment scripts" -ForegroundColor Cyan
