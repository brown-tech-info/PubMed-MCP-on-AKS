<#
.SYNOPSIS
    Creates the server.py file with the PubMed MCP Server implementation
.DESCRIPTION
    This script creates the core MCP server file that handles PubMed API interactions
#>
...
Write-Host "🐍 Creating server.py - PubMed MCP Server..." -ForegroundColor Green

# Server.py content with your original implementation
$serverContent = @'
#!/usr/bin/env python3
"""
PubMed MCP Server
...
$fullPath = Join-Path $WorkspacePath "server.py"
Set-Content -Path $fullPath -Value $serverContent -Encoding UTF8

Write-Host "✅ Created: server.py - PubMed MCP Server (Complete implementation)" -ForegroundColor Green
...
Write-Host "   • Search PubMed publications" -ForegroundColor White
Write-Host "   • Get publication details by PMID" -ForegroundColor White
Write-Host "   • Find similar articles" -ForegroundColor White
Write-Host "   • Comprehensive XML parsing" -ForegroundColor White
Write-Host "   • Enhanced error handling" -ForegroundColor White
Write-Host "   • Rich markdown formatting" -ForegroundColor White

Write-Host "`n🎉 server.py created successfully!" -ForegroundColor Green
Write-Host "📋 Next: Create app.py (FastAPI wrapper)" -ForegroundColor Cyan